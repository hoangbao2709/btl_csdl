...
export default function Input() {
  ...
  const [submittedInvester, setSubmittedInvester] = useState("");
  ...
  useEffect(() => {
    ...
    if (data && data.length > 0 ) {
      const initialValue = data[0].inventory;
      setSubmittedInvester(initialValue);
    }
    if (data && data.length > 0 ) {
  ...
  const handleChange = (event) => {
    ...
    if (name === "invester") setSubmittedInvester(value);
